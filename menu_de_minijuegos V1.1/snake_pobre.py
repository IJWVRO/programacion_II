...
pygame.init()
# Intenta inicializar mixer; si falla, continúa (ya estaba en el original)
try:
    pygame.mixer.init()
except Exception:
    pass
...
try:
    ...
    # --- Música de Fondo (CAMBIOS RECIENTES: Línea comentada) ---
...
def send_email(player_name, to_email, new_score):
    sender_email = "fairbotnotifier@gmail.com"  # Cambiar por tu email si querés
    sender_password = "pvrcyhncquzcibyv"  # Generar en Gmail (app password)

    msg = EmailMessage()
    msg['Subject'] = f"Nuevo récord en Retro Snake Plus: {new_score}"
    msg['From'] = sender_email
    msg['To'] = to_email
    msg.set_content(f"¡Felicidades {player_name}! Has alcanzado un nuevo récord de {new_score} puntos en Retro Snake Plus.")
...
class DataManager:
    ...
    def update_score(self, name, email, score):
        ...
        y envía email si hay un récord.
        """
        if not name or name == "Invitado":
            return False, 0
        ...
        for player in self.data:
            if player['name'].lower() == name.lower():
                ...
                if score > old_score:
                    ...
                    # ---- ENVÍO DE EMAIL ----
                    if email.strip():  # Solo si hay email
                        try:
...
class Food:
    ...
    def __init__(self, snake_body, walls, num_foods=4):
        self.positions = [] # Lista para guardar las posiciones de comida
    ...
    def draw(self):
        ...
        for position in self.positions:
            food_rect = pygame.Rect(
                OFFSET + int(position.x) * cell_size,
                OFFSET + int(position.y) * cell_size,
                cell_size,
                cell_size
            )
            try:
...
class Snake:
    ...
    def draw(self):
        ...
        for index, segment in enumerate(self.body):
            segment_rect = (
                OFFSET + int(segment.x) * cell_size,
                OFFSET + int(segment.y) * cell_size,
                cell_size,
                cell_size
            )
...
class Game:
    ...
    def draw_walls(self):
        ...
        for wall in self.walls:
            wall_rect = (
                OFFSET + int(wall.x) * cell_size,
                OFFSET + int(wall.y) * cell_size,
                cell_size,
                cell_size
            )
        ...
        for index, position in enumerate(self.food.positions):
            if head == position:
                ...
                if self.score > 0 and self.score % 100 == 0:
                    self.walls = self.generate_walls()
                    # Regenera TODAS las comidas en nuevas posiciones seguras
    ...
    def check_collision_with_walls(self):
        ...
        # comparaciones con Vector2 en la lista walls están bien porque walls usa Vector2
    ...
    def game_over(self):
        ...
        self.walls = self.generate_walls()
        try:
            self.food.generate_initial_pos(self.snake.body, self.walls) # Regenera las comidas
        except Exception:
            pass
        self.score = 0
...
def draw_leaderboard():
    ...
    for i, player in enumerate(current_page_scores):
        pos = start_index + i + 1
        text_line = f"{pos:<3}  | {player['name'][:10]:<10} | {player.get('score',0)}"
...
def draw_user_input():
    ...
    # aquí el ancho podría ser mayor si querés, mantuve 300 pero con límite de 80 caracteres
    ...
    # Si el email excede el ancho de la caja, lo recortamos visualmente con '...' (solo visual)
    display_email = email_text
    max_chars_visible = 36
    if len(display_email) > max_chars_visible:
        display_email = "..." + display_email[-(max_chars_visible-3):]

    email_surf = input_font.render(display_email, True, BLACK)
        ...
        if event.key == pygame.K_RETURN:
            ...
            if len(char) == 1 and char.isprintable():
                if active_input == 'name' and len(name_text) < 15:
                    name_text += char
                # <-- AUMENTADO: permitimos emails más largos (hasta 80 chars) -->
                elif active_input == 'email' and len(email_text) < 80:
                    email_text += char
            
def validate_email(email):
    """Verifica si el email tiene un formato básico válido."""
    # Regex más flexible que permite subdominios y tlds de longitud variable
    regex = r'^[\w\.\+\-]+@[\w\.\-]+\.\w{2,}$'
...
def draw_top_hud(score_value):
    """Dibuja un HUD centrado arriba con el puntaje (nuevo según tu pedido)."""
    hud_w = 360
    hud_h = 48
    hud_x = (ANCHO - hud_w) // 2
    hud_y = 14  # "arriba" con un pequeño margen

    # Surface translúcido para el HUD
    hud_surf = pygame.Surface((hud_w, hud_h), pygame.SRCALPHA)
    hud_surf.fill((10, 10, 10, 200))  # fondo semi-transparente
    pygame.draw.rect(hud_surf, DARK_GREEN, (0, 0, hud_w, hud_h), 2, border_radius=8)

    score_text = score_font.render(f"Puntaje: {score_value}", True, WHITE)
    hud_surf.blit(score_text, score_text.get_rect(center=(hud_w//2, hud_h//2)))
...
while True:
    ...
    if game_state == MENU:
        ...
        # --- DIBUJO HUD SUPERIOR (centrado y arriba) ---
        draw_top_hud(game.score)

        # 3. Dibuja Score y Nombre (en la parte inferior como antes)

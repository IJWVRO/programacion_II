...
def main():
    ...
    # Pedir nombre al inicio (opcional, se pedirá de nuevo al morir)
    player_name = get_player_initial_name()
...
def get_player_initial_name():
    # pedir un nombre inicial pero no obligatorio (si cancela devolvemos "Jugador")
    pygame.display.set_caption('Flappy Pobre - Ingresa nombre (opcional)')
    name = prompt_text_box("Ingresa tu nombre (opcional)", hint="Presiona ENTER para aceptar o ESC para omitir", max_len=15)
    if name is None:
        return "Jugador"
    return name
...
def showGameOverScreen(crashInfo, initial_player_name):
    """
    ...
    name, email = get_name_and_email(score, default_name=initial_player_name)
    send_ok = None
    send_err = ""
    used_name = initial_player_name
    if name is None:
        # usuario cancelo -> usar nombre inicial y no enviar correo
        used_name = initial_player_name if initial_player_name else "Jugador"
    else:
        used_name = name
    ...
    # Si el jugador no ingreso nombre (o cancelo), igualmente actualizamos la tabla con initial_player_name
    if name is None:
        update_high_scores(score, used_name)
